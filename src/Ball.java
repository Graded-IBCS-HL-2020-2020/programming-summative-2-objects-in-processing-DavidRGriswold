class Ball {
    ...
    /**
     * SUMMATIVE OPTIONAL Examine the default constructor below and tweak it to make
     * the position, speed, and/or color random as well!
     */
    public Ball(Sketch s) {
        sketch = s;
        // the line below chooses a random number between 50 and 150 and sets diameter
        // to it
        diameter = sketch.random(50, 150);
        x = sketch.width / 2; // puts the ball right in the middle
        y = 100;
        col = sketch.color(255, 0, 0); // red
        speedX = 1;
        speedY = 3;
    }
    ...
    public Ball(Sketch s, float X, float Y, float ballDiam, int ballColor, float sx, float sy) {
        /*
         * SUMMATIVE REQUIRED Fix this constructor so that it correctly sets all of the
         * instance variables to the set values
         */
    }
